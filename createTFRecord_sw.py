...
def loadImage(path, obj):
    ...
    if h > w:
        diff = h-w
        if((int(obj[3]) + diff/2) <= width):
            obj[3] = int(obj[3]) + diff/2
        if((int(obj[1]) - diff/2) >= 0):    
            obj[1] = int(obj[1]) - diff/2
    if h < w:
        diff = w-h
        if((int(obj[4]) + diff/2) <= height):
            obj[4] = int(obj[4]) + diff/2
        if((int(obj[2]) - diff/2) >= 0):
...
def writeTFRecords(xs, ys):
    ...
    train_filename = 'output/train.tfrecords'
    ...
    for line in content:
        ...
        for i in range(50):
            ...
            label = 2
            feature = {'label': _int64_feature(label),
                   'image': _bytes_feature(tf.compat.as_bytes(crop_img.tostring()))}  
            #Ein TFExample wird aus dem Feature erstellt
            example = tf.train.Example(features=tf.train.Features(feature=feature))
            #Das TFExample wird in die TFRecordsdatei geschrieben
            writer.write(example.SerializeToString())
